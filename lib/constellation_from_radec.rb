class ConstellationfromRadec
  ...
  def constellation
    converted_ra, converted_dec = precess(right_ascension, declination, EPOCH, REF_EPOCH)
  ...
  def precess(ra1, dec1, epoch1, epoch2)
    ...
    a = CSR * t * (
      23042.53 + st * (139.75 + 0.06 * st) + t * (30.23 - 0.27 * st + 18.0 * t)
    )
    b = CSR * t * t * (79.27 + 0.66 * st + 0.32 * t) + a
    c = CSR * t * (
      20046.85 - st * (85.33 + 0.37 * st) + t * (-42.67 - 0.37 * st - 41.8 * t)
    )
  ...
  def find_boundary(ra, dec)
    BOUNDARIES.detect do |boundary|
      (dec >= boundary[2] && ra >= boundary[0] && ra < boundary[1])
    end
  end
end

pp ConstellationfromRadec.new(2.52975, 89.264109).constellation
pp ConstellationfromRadec.new(6.752481, -16.716116).constellation
pp ConstellationfromRadec.new(7.755277, 28.026199).constellation
pp ConstellationfromRadec.new(18.615640, 38.783692).constellation
